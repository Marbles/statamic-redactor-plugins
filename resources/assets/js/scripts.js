		...
		return {
			...
			insert: function()
			{
				...
				for (var i = 0; i < rows; i++)
				{
					...
					for (var z = 0; z < columns; z++)
					{
						...
						if (i === 0 && z === 0)
						{
							$column.append(this.caret.getCoords());
						}
				...
				if ($(current).closest('li', this.core.getEditor()[0]).length !== 0)
				{
			...
			deleteRow: function()
			{
				...
				if ($focus_tr.length)
				{
					...
					if ($focus_td.length)
					{
						$focus_td.prepend(this.caret.getCoords());
					}
				}
			...
			deleteColumn: function()
			{
				...
				$table.find('tr').each($.proxy(function(i, elem)
				{
					...
					if (i === 0)
					{
						$elem.find('td, th').eq(focusIndex).prepend(this.caret.getCoords());
					}
